...
sqlmap 是一个开源的渗透测试工具，可用于SQL注入漏洞以及数据库服务器权限获取的自动化检测。它具有功能强大的检测引擎,针对各种不同类型数据库的渗透测试的功能选项，包括获取数据库中存储的数据，访问操作系统文件甚至可以通过外带数据连接的方式执行操作系统命令。

**sqlmap项目由[Netsparker Web Application Security Scanner](https://www.netsparker.com/scan-website-security-issues/?utm_source=sqlmap.org&utm_medium=banner&utm_campaign=github)赞助。**
...
你可以从 [这里](https://asciinema.org/a/46601) 看到一个sqlmap 的使用样例。除此以外，你还可以查看 [使用手册](https://github.com/sqlmapproject/sqlmap/wiki/Usage)，以了解sqlmap支持的所有特性、参数、命令行选项开关及说明的使用帮助。
